...
def cal_expected_interval_single(single_visitor_result):
    ...
    pred_revisit_interval = np.sum(rint * revisit_prob)  # Predicted revisit interval for single user
...
def cal_expected_interval(result):
    ...
    pred_revisit_interval = np.sum(rint * revisit_prob, axis=1)  # Predicted revisit interval for all users
