...
from tensorflow.python.keras.layers import Layer, LSTM, Reshape, Dense, Embedding, Lambda, BatchNormalization

# import keras
# import keras.backend as K
# import tensorflow as tf
# # from keras.backend.tensorflow_backend import set_session
# from keras.layers import Layer, TimeDistributed, LSTM, CuDNNGRU, Reshape, Conv1D, Concatenate, Dense, Embedding, Lambda
...
"""
* Filename: aaai19deprecated.py - FOR PRACTICE PURPOSE
...
class AAAI19():
    ...
    def generate_model(self):
        ...
        concat = tf.keras.layers.Concatenate()([user_input, area_input, visit_features_input])
        print(concat)
        logits = Dense(1, activation='softmax')(concat)
        ...
        # out1 = Lambda(uc_c_loss_ce)(logits)
        # out2 = Lambda(uc_c_loss_ce)(logits)
        # self.model = keras.Model(inputs=single_input, outputs=[out1, out2])


    def generate_light_model(self):
        """Lighter version of AAAI19 model"""
        ...
        class AddTimeAscInputLayer2(Layer):
            """These set of computation did not work without defining this layer"""
            def __init__(self, **kwargs):
                super(AddTimeAscInputLayer2, self).__init__(**kwargs)

            def build(self, input_shape):
                super(AddTimeAscInputLayer2, self).build(input_shape)
            ...
            def call(self, x):
                ...
                yseq = K.variable(np.expand_dims(np.array(range(365)) + 0.5, 0))
                yseq = K.dot(ones, yseq)
                yseq = Lambda(lambda x: K.permute_dimensions(x, (0, 2, 1)))(yseq)
                print('yseq', yseq)
                all_areas_lstm = LSTM(64, return_sequences=True)(yseq)
                concat2 = Concatenate()([all_areas_lstm, expinp])
                return concat2

            def compute_output_shape(self, input_shape):
                return (input_shape[0], input_shape[1], input_shape[2]+64)

        concat2 = AddTimeAscInputLayer2()(expinp)

        logits = Dense(1, activation='softmax')(concat2)
        logits = Lambda(lambda x: K.squeeze(x, axis=-1))(logits)
        self.model = keras.Model(inputs=single_input, outputs=logits)
        print(self.model.summary())
