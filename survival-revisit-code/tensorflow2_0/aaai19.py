...
from keras.layers import Layer, TimeDistributed, LSTM, CuDNNGRU, Reshape, Conv1D, Concatenate, Dense, Embedding, Lambda, Flatten
...
"""
* Filename: aaai19.py (New Loss Version)
...
class AAAI19Loss():
    ...
    def aaai19_loss(self):
        ...
        self.tmp_tensor = K.concatenate([self.y_pred, self.y_true], axis=-1)   # 이걸 하려면 정답 y_true가 필요해, model에 넣기엔 정답이 지금 없어.
        print(self.y_true)
        print(self.y_pred)
...
class AAAI19():
    ...
    def generate_model(self):
        ...
        concat = Dense(128, activation=keras.activations.softmax)(concat)
        concat = BatchNormalization()(concat)
        ...
        def cal_expected_interval_for_loss(y_pred):
            """ For calculating MSE loss for censored data, we have to get the predict revisit interval."""
            zeros = K.zeros(shape=(FLAGS.batch_size, 1))
            tmp_concat = K.concatenate([zeros, y_pred], axis=1)
            surv_rate = K.ones_like(tmp_concat) - tmp_concat
        ...
        def uc_loss_nll(self, uc_loss_nll_option='date'):
            """Wrapper function for all negative log-likelihood loss"""
            probs_survive = K.map_fn(
                functools.partial(calculate_proba, interval=self.d_interval[uc_loss_nll_option]),
                elems=self.tmp_tensor)
            prob_revisit_at_z = tf.transpose(probs_survive)[0] - tf.transpose(probs_survive)[1]
            # If censored -> multiply by 0 -> thus ignored
            prob_revisit_at_z_uncensored = tf.add(tf.multiply(prob_revisit_at_z, tf.transpose(probs_survive)[-1]),
                                                  1e-20)
            return -tf.reduce_sum(K.log(prob_revisit_at_z_uncensored))  # / num_revisitors_in_batch
        ...
        # rmse
        def preprocess1(y_pred):
            """ For calculating MSE loss for censored data, we have to get the predict revisit interval."""
            zeros = K.zeros_like(y_pred[:, :1])  # [None, 1]
            tmp_concat = K.concatenate([zeros, y_pred], axis=1)  # [None, 366]
            surv_rate = K.ones_like(tmp_concat) - tmp_concat  # [None, 366]
            surv_prob = K.cumprod(surv_rate, axis=1)  # [None, 1]
            revisit_prob = surv_prob[:, :-1] * y_pred  # [None, 365]

            aa = K.ones_like(y_pred[:, :1])  # [None, 1]
            avg_bin = (np.array(range(365)) + 0.5).astype('float32') # [365, ]
            bb = K.expand_dims(avg_bin, axis=0) # [1, 365]
            rint = K.dot(aa, bb)  # [None, 365]

            # pred_revisit_probability = K.sum(revisit_prob, axis=1)
            pred_revisit_interval = K.sum(rint * revisit_prob, axis=1) # [None, ]
            out = K.stack([pred_revisit_interval, pred_revisit_interval], axis=-1) # [None, 2]  The second column is dummy.
            return out

        # ce
        def preprocess2(hrate):
            # probs_survive =
            # * cross entropy를 계산하고 싶은데
            return hrate



        # LSTM
        all_areas_lstm = CuDNNGRU(64, return_sequences=True)(concat2)
        print('concat2', concat2)
        # logits = Dense(1, activation='softmax')(all_areas_lstm)
        logits = TimeDistributed(Dense(1, activation='softmax'))(all_areas_lstm)
        logits = Lambda(lambda x: K.squeeze(x, axis=-1))(logits)
        print('logits', logits)
        out1 = Lambda(preprocess1)(logits)
        print('out1', out1)
        # out2 = Lambda(preprocess2)(logits)
        self.model = keras.Model(inputs=single_input, outputs=out1)
        print(self.model.summary())
    ...
    def train_test(self):
        ...
        def ex_loss(y_true, y_pred):
            # y_pred = pred_revisit_print(y_true)
            print('ypred', y_pred) # [None, 2]
            print('ytrue', y_true) # [None, 2]

            squared_error_all = K.square(keras.layers.Subtract()([y_pred[:, -1], y_true[:, -1]]))    #[None, ]
            print('squared', squared_error_all)
            squared_error_uc = tf.multiply(squared_error_all, y_true[:, -2])  # y_true[:, -2] is a binary revisit label.
            num_revisitors_in_batch = K.sum(y_true[:, -2])
            print('num_revisitors', num_revisitors_in_batch)
            loss = K.sqrt(K.sum(squared_error_uc) / num_revisitors_in_batch)
            print('loss', loss)
            # print(K.eval(loss))
            return loss

        print('!!!', K.eval(ex_loss(K.variable(np.random.rand(32, 2)), K.variable(np.random.rand(32, 2)))))
        # Compile model
        self.model.compile(optimizer=keras.optimizers.Adam(0.001),
                           loss=ex_loss,
                           # loss=[ex_loss, ex_loss], # aaai19_loss
                           # loss_weights=[0.2, 0.8]   # rmse_loss
                           )
