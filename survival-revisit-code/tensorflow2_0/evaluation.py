...
from wsdm17nsrtensorflow2 import WSDM
from aaai19tensorflow2 import AAAI19
...
class Evaluation():
    ...
    def calculate_metrics(self, data, y_pred_regr_all, test_case, name):
        ...
        if test_case == 'test':
            ...
            cindex = lifelines.utils.concordance_index(event_times=data.df_test['suppress_time'],
                                                       predicted_scores=y_pred_regr_all,
                                                       event_observed=(data.df_test['revisit_intention'] == 1))
            ...
            cindex = lifelines.utils.concordance_index(event_times=data.df_train_censored['suppress_time'],
                                                       predicted_scores=y_pred_regr_all,
                                                       event_observed=(data.df_train_censored[
                                                                           'revisit_intention'] == 1))
    ...
    def naive_baseline(self, data):
        ...
        golden_binary = data.df_test['revisit_intention']
        ...
        golden_suppress_time = data.df_test['suppress_time']
        self.log_rslt['test']['cindex_majority_baseline'] = lifelines.utils.concordance_index(
            event_times=golden_suppress_time,
            predicted_scores=np.full(golden_suppress_time.shape, np.mean(data.df_train['suppress_time'])),
            event_observed=(golden_binary == 1))

        revisited_indices = np.argwhere(~np.isnan(data.df_test['revisit_interval']))
        golden_revisit_interval = np.array(data.df_test['revisit_interval'])[revisited_indices]
        rmse_maj_revisit_interval = np.full(golden_revisit_interval.shape, np.mean(data.df_train['suppress_time']))
    ...
    def naive_baseline_for_train_censored(self, data):
        ...
        golden_binary = data.df_train_censored[
        ...
        self.log_rslt['train_censored']['fscore_majority_baseline'] = sklearn.metrics.f1_score(y_true=golden_binary,
                                                                                               y_pred=y_binary_majority_voting)
        golden_suppress_time = data.df_train_censored['suppress_time']
        # Use average suppress time as a prediction result.
        ...
        revisited_indices = np.argwhere(~np.isnan(data.df_train_censored['revisit_interval']))
        golden_revisit_interval = np.array(data.df_train_censored['revisit_interval'])[revisited_indices]
        rmse_maj_revisit_interval = np.full(golden_revisit_interval.shape, np.mean(data.df_train['suppress_time']))
        rmse_golden_avg_revisit_interval = np.full(golden_revisit_interval.shape, np.mean(golden_revisit_interval))
    ...
    def poisson_process_baseline(self, data):
        ...
        for test_case in ['test', 'train_censored']:
            ...
            if test_case == 'test':
                ...
                print('Poisson_test: ', np.round(list(y_pred_regr_all[:10]), 0))
            ...
            # Case for train_censored instances for their prediction
                ...
                y_pred_regr_all = mu.apply(np.random.exponential) / 86400
                print('Poisson_train_censored: ', np.round(list(y_pred_regr_all[:10]), 0))
    ...
    def hawkes_process_baseline(self, data):
        ...
        print('Hawkes_test: ', np.round(y_pred_regr_all_test[:10]))
        ...
        print('Hawkes_train_censored: ', np.round(y_pred_regr_all_train_censored[:10]))
    ...
    def icdm_baseline(self, data):
        ...
        for task in ['regr', 'clas']:
            if task == 'regr':
                ...
                print('ICDM_test: ', np.round(y_pred_regr[:10]))
                rmse = utils.root_mean_squared_error(y_test_regr, y_pred_regr)
    ...
    def icdm_baseline_for_train_censored(self, data, task='regr'):
        ...
        for task in ['regr', 'clas']:
            if task == 'regr':
                ...
                y_pred_regr = clf_regr.predict(X_test_regr)
                print('ICDM_train_censored: ', np.round(y_pred_regr[:10]))
                rmse = utils.root_mean_squared_error(y_test_regr, y_pred_regr)
                ...
                cindex = lifelines.utils.concordance_index(event_times=data.df_train_censored['suppress_time'],
                                                   predicted_scores=y_pred_regr_all,
                                                   event_observed=(data.df_train_censored['revisit_intention'] == 1))
            if task == 'clas':
                # Dataset for Classification task: All data (censored + uncensored data)
                df_train_clas = data.df_train
    ...
    def traditional_survival_analysis_baseline(self, data):
        ...
        try:
            ...
            remaining_attributes.remove('day_hour_comb')
            ...
            traceback.print_exc()
            print('CPH result is NaN- test')
            self.log_rslt['test']['acc_cph_baseline'] = np.nan
            self.log_rslt['test']['fscore_cph_baseline'] = np.nan
            self.log_rslt['test']['cindex_cph_baseline'] = np.nan
            self.log_rslt['test']['RMSE_cph_baseline'] = np.nan
    ...
    def traditional_survival_analysis_baseline_for_train_censored(self, data):
        ...
        try:
            ...
            remaining_attributes = list(data.handcrafted_features) + ['suppress_time', 'revisit_intention']
            remaining_attributes.remove('day_hour_comb')
            remaining_attributes.remove('occ_visit_ratio')
            df_train = data.df_train[remaining_attributes]
            df_test = data.df_test[remaining_attributes]
            ...
            print('CPH result is NaN- train-censored')
            self.log_rslt['train_censored']['acc_cph_baseline'] = np.nan
            self.log_rslt['train_censored']['fscore_cph_baseline'] = np.nan
            self.log_rslt['train_censored']['cindex_cph_baseline'] = np.nan
            self.log_rslt['train_censored']['RMSE_cph_baseline'] = np.nan

    def WSDM17_baseline(self, data):
        wsdm = WSDM(store_id=FLAGS.store_id, GPU_id="0", for_baseline=True)
        wsdm.run()
        self.evaluate(data, wsdm.pred_test, algo='WSDM17', show_console=False)
        self.evaluate_train_censored(data, wsdm.pred_train_censored, algo='WSDM17', show_console=False)


    def AAAI19_baseline(self, data):
        aaai = AAAI19(store_id=FLAGS.store_id, GPU_id="0", for_baseline=True)
        aaai.run()
        self.evaluate(data, aaai.pred_test, algo='AAAI19', show_console=False)
        self.evaluate_train_censored(data, aaai.pred_train_censored, algo='AAAI19', show_console=False)


    @timeit
    def evaluate(self, data, pred_result, algo='unknown', show_console=True):
        """ Evaluation results """
        ...
        print('# SurvRev-hazard-top10', np.around(pred_result, 4)[0, :10])
        print('# SurvRev-hazard-mid10', np.around(pred_result, 4)[0, 180:190])
        print('# SurvRev-hazard-fin10', np.around(pred_result, 4)[0, -10:])
        print('# SurvRev-pred-interval', np.around(pred_revisit_interval, 4)[:10])
        print('# Real test int', np.around(np.array(data.df_test['revisit_interval']), 4)[:10])
        print('# Real test sup', np.around(np.array(data.df_test['suppress_time']), 4)[:10])
        print('# Real test bin', np.around(np.array(data.df_test['revisit_intention']), 4)[:10])
        ...
        golden_binary = data.df_test['revisit_intention']  # golden_binary: revisit binary labels
        ...
        self.log_rslt['test']['acc_'+algo+''] = sklearn.metrics.accuracy_score(y_true=golden_binary, y_pred=y_binary_pred)
        if show_console:
            self.print_results_console(test_case='test', metric='acc')

        """F-score: """
        self.log_rslt['test']['fscore_'+algo+''] = sklearn.metrics.f1_score(y_true=golden_binary, y_pred=y_binary_pred)
        if show_console:
        ...
        self.log_rslt['test']['cindex_'+algo+'_interval'] = lifelines.utils.concordance_index(event_times=golden_suppress_time,
                                                   predicted_scores=pred_revisit_interval,
                                                   event_observed=(golden_binary == 1))
        self.log_rslt['test']['cindex_'+algo+'_censored_prob'] = lifelines.utils.concordance_index(event_times=golden_suppress_time,
        ...
        self.log_rslt['test']['RMSE_'+algo+''] = utils.root_mean_squared_error(y_true=golden_revisit_interval,
                                                 y_pred=pred_revisit_interval[revisited_indices])
        if show_console:
            self.print_results_console(test_case='test', metric='RMSE')

    @timeit
    def evaluate_train_censored(self, data, pred_result, algo='unknown', show_console=True):
        """ Evaluation results for censored data"""
        print('\n----Final evaluation results for censored data from training set----')
        print('Train size: {}, Train Censored size: {}'.format(len(data.train_visits), len(data.df_train_censored)))
        ...
        print('# SurvRev-hazard-mid10', np.around(pred_result, 4)[0, 180:190])
        print('# SurvRev-hazard-fin10', np.around(pred_result, 4)[0, -10:])
        print('# SurvRev-pred-int', np.around(pred_revisit_interval, 4)[:10])
        print('# Real tc int', np.around(np.array(data.df_train_censored['revisit_interval']), 4)[:10])
        print('# Real tc sup', np.around(np.array(data.df_train_censored['suppress_time']), 4)[:10])
        print('# Real tc bin', np.around(np.array(data.df_train_censored['revisit_intention']), 4)[:10])
        print('1) Censored + Uncensored')
        print('  i) Binary classification')
        # Accuracy
        golden_binary = data.df_train_censored['revisit_intention']  # golden_binary: revisit binary labels
        # True if the predicted revisit time is in the test dataframe, otherwise False.
        y_binary_pred = data.train_censored_visits.ts_end + (86400 * pred_revisit_interval.reshape(-1)) <= data.last_timestamp
        self.log_rslt['train_censored']['acc_'+algo] = sklearn.metrics.accuracy_score(y_true=golden_binary, y_pred=y_binary_pred)
        if show_console:
            self.print_results_console(test_case='train_censored', metric='acc')

        # F-score
        print('pred_rev_int', np.around(pred_revisit_interval, 4)[:20])
        self.log_rslt['train_censored']['fscore_'+algo] = sklearn.metrics.f1_score(y_true=golden_binary, y_pred=y_binary_pred)
        if show_console:
        ...
        self.log_rslt['train_censored']['cindex_'+algo+'_interval'] = lifelines.utils.concordance_index(
            event_times=golden_suppress_time,
            predicted_scores=pred_revisit_interval,
            event_observed=(golden_binary == 1))
        self.log_rslt['train_censored']['cindex_'+algo+'_censored_prob'] = lifelines.utils.concordance_index(
            event_times=golden_suppress_time,
            predicted_scores=censored_probability,
            event_observed=(golden_binary == 1))
        if show_console:
        ...
        revisited_indices = np.argwhere(~np.isnan(data.df_train_censored['revisit_interval']))
        golden_revisit_interval = np.array(data.df_train_censored['revisit_interval'])[revisited_indices]
        self.log_rslt['train_censored']['RMSE_'+algo] = utils.root_mean_squared_error(y_true=golden_revisit_interval,
                                                                           y_pred=pred_revisit_interval[
                                                                               revisited_indices])
        if show_console:
            self.print_results_console(test_case='train_censored', metric='RMSE')

    def callback_test_evaluate(self, data, pred_result, algo='unknown'):
        ...
        golden_binary = data.df_test['revisit_intention']  # golden_binary: revisit binary labels
        y_binary_pred = data.test_visits.ts_end + (86400 * pred_revisit_interval.reshape(-1)) <= data.last_timestamp
        self.log_rslt['callback_test']['acc_'+algo] = sklearn.metrics.accuracy_score(y_true=golden_binary, y_pred=y_binary_pred)
        self.log_rslt['callback_test']['fscore_'+algo] = sklearn.metrics.f1_score(y_true=golden_binary, y_pred=y_binary_pred)
        print('      - Accuracy: {:.4} (Our Model)'.format(self.log_rslt['callback_test']['acc_'+algo]))
        print('      - F-score: {:.4} (Our Model)'.format(self.log_rslt['callback_test']['fscore_'+algo]))

        # C-index (for test)
        golden_suppress_time = data.df_test['suppress_time']
        self.log_rslt['callback_test']['cindex_'+algo+'_interval'] = lifelines.utils.concordance_index(
            event_times=golden_suppress_time,
            predicted_scores=pred_revisit_interval,
            event_observed=(golden_binary == 1))
        print('      - C-index: {:.4} (Our Model)'.format(self.log_rslt['callback_test']['cindex_'+algo+'_interval']))

        # RMSE (for test)
        revisited_indices = np.argwhere(~np.isnan(data.df_test['revisit_interval']))
        golden_revisit_interval = np.array(data.df_test['revisit_interval'])[revisited_indices]
        self.log_rslt['callback_test']['RMSE_'+algo] = utils.root_mean_squared_error(y_true=golden_revisit_interval, y_pred=pred_revisit_interval[revisited_indices])
        print('      - RMSE: {:.4} (Our Model)'.format(self.log_rslt['callback_test']['RMSE_'+algo]))

    def callback_train_censored_evaluate(self, data, pred_result, algo='unknown'):
        ...
        golden_binary = data.df_train_censored['revisit_intention']
        y_binary_pred = data.train_censored_visits.ts_end + (86400 * pred_revisit_interval.reshape(-1)) <= data.last_timestamp
        self.log_rslt['callback_train_censored']['acc_'+algo] = sklearn.metrics.accuracy_score(y_true=golden_binary, y_pred=y_binary_pred)
        self.log_rslt['callback_train_censored']['fscore_'+algo] = sklearn.metrics.f1_score(y_true=golden_binary, y_pred=y_binary_pred)
        print('      - Accuracy: {:.4} (Our Model)'.format(self.log_rslt['callback_train_censored']['acc_'+algo]))
        print('      - F-score: {:.4} (Our Model)'.format(self.log_rslt['callback_train_censored']['fscore_'+algo]))

        # C-index (for train_censored)
        golden_suppress_time = data.df_train_censored['suppress_time']
        self.log_rslt['callback_train_censored']['cindex_'+algo+'_interval'] = lifelines.utils.concordance_index(
            event_times=golden_suppress_time,
            predicted_scores=pred_revisit_interval,
            event_observed=(golden_binary == 1))
        print('      - C-index: {:.4} (Our Model)'.format(
            self.log_rslt['callback_train_censored']['cindex_'+algo+'_interval']))

        # RMSE (for train_censored)
        revisited_indices = np.argwhere(~np.isnan(data.df_train_censored['revisit_interval']))
        golden_revisit_interval = np.array(data.df_train_censored['revisit_interval'])[revisited_indices]
        self.log_rslt['callback_train_censored']['RMSE_'+algo] = utils.root_mean_squared_error(y_true=golden_revisit_interval, y_pred=pred_revisit_interval[revisited_indices])
        print('      - RMSE: {:.4} (Our Model)'.format(self.log_rslt['callback_train_censored']['RMSE_'+algo]))
    ...
    def print_results_console(self, test_case, metric):
        ...
        def _if_exist_then_print(metric, key_name, print_name):
            ...
            try:
                print('      - {}: {:.4} ({})'.format(full_name, float(self.log_rslt[test_case][key]), print_name))
            except KeyError:
                pass
        ...
        if metric != 'cindex':
            _if_exist_then_print(metric, key_name='WSDM17', print_name='WSDM17')
            _if_exist_then_print(metric, key_name='AAAI19', print_name='AAAI19')
            _if_exist_then_print(metric, key_name='SurvRev', print_name='SurvRev')
        else:

            _if_exist_then_print(metric, key_name='WSDM17_interval',
                                 print_name='WSDM17 - calculated by pred_revisit_interval')
            _if_exist_then_print(metric, key_name='WSDM17_censored_prob',
                                 print_name='WSDM17 - calculated by pred_revisit_probability')
            _if_exist_then_print(metric, key_name='AAAI19_interval',
                                 print_name='AAAI19 - calculated by pred_revisit_interval')
            _if_exist_then_print(metric, key_name='AAAI19_censored_prob',
                                 print_name='AAAI19 - calculated by pred_revisit_probability')
            _if_exist_then_print(metric, key_name='SurvRev_interval', print_name='SurvRev - calculated by pred_revisit_interval')
            _if_exist_then_print(metric, key_name='SurvRev_censored_prob', print_name='SurvRev - calculated by pred_revisit_probability')
    ...
    def print_output(self, additional_inputs_all):
        ...
        with open(FLAGS.all_result_file_path, 'a') as ff:
            ...
            if exists:
                with open(FLAGS.all_result_file_path, newline='') as fr:
