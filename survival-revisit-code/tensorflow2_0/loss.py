...
class CustomLoss():
    ...
    def cal_expected_interval_for_loss(self, y_pred):
        ...
        zeros = K.zeros(shape=(len(y_pred), 1))
        ...
        rint = K.variable(np.stack([np.array(range(365)) + 0.5 for _ in range(len(y_pred))], axis=0))
    ...
    def combined_loss(self):
        ...
        combined_loss = 1
        if self.c_nll_date > 0:
            combined_loss *= self.c_nll_date * self.uc_loss_nll_date()
        if self.c_nll_week > 0:
            combined_loss *= self.c_nll_week * self.uc_loss_nll_week()
        if self.c_nll_month > 0:
            combined_loss *= self.c_nll_month * self.uc_loss_nll_month()
        if self.c_nll_season > 0:
            combined_loss *= self.c_nll_season * self.uc_loss_nll_season()
        if self.c_rmse > 0:
            combined_loss *= self.c_rmse * self.uc_loss_rmse()
        if self.c_ce > 0:
            combined_loss *= self.c_ce * self.uc_c_loss_ce()
        if self.c_rank > 0:
            combined_loss *= self.c_rank * self.uc_c_loss_rank()
