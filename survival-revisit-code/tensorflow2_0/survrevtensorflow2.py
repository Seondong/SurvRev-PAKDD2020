...
from tensorflow.keras.layers import Masking, Input, TimeDistributed, Dropout, LSTM, GRU, Reshape, Conv1D, Concatenate, Dense, Embedding, Lambda, Flatten, BatchNormalization, subtract, Bidirectional
import warnings
...
class lowLevelEncoder(Model):
    ...
    def call(self, single_input):
        ...
        mask = Masking(mask_value=self.data.area_embedding['pad'], input_shape=(self.max_num_areas, FLAGS.embedding_dim)).compute_mask(areas_emb)
        areas_emb = Bidirectional(LSTM(units=FLAGS.emb_dim_uid, return_sequences=True))(areas_emb, mask=mask)
        # areas_emb = Bidirectional(LSTM(units=FLAGS.emb_dim_uid, return_sequences=True))(areas_emb)
